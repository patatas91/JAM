...
public class ADSLImpl extends UnicastRemoteObject implements ADSL {
	...
	// Listado con los RemoteMessageBox
	...
	public List<RemoteMessageBox> getRemoteMessageBox(AgentID agentID)
		...
		// Solo si esta activo el monitor
		try {
	...
	public void insertRemoteMessageBox(RemoteMessageBox messageBox)
			throws JAMADSLException, RemoteException {
			...
			while (i.hasNext()) {
				...
				try {
					if (box.getOwner().equals(owner)) {
						throw new JAMADSLException("Box ya presente");
					}
				} catch (RemoteException e) {
					// Objeto no activo
					i.remove();
	...
	public void removeRemoteMessageBox(AgentID agentID) throws RemoteException,
		...
		if (!borrado) {
			throw new JAMADSLException(
					"No se ha borrado porque no se ha encontrado ningun mensaje que cumpla requisitos");
		}
		System.out
